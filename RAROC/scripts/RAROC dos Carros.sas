...
CREATE TABLE HISTORICO_MES AS 
   ...
   ( SELECT NR_CPF_CNPJ, 
			...
			AVG(VR_TRN)      AS AVG_VR_TRN ,
            PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY VR_TRN) AS P25_VR_TRN ,
			PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY VR_TRN) AS P75_VR_TRN 
...
PROC SQL;
   CONNECT TO NETEZZA AS con1 (DATABASE=PRD_DWM SERVER=NTZP110 authdomain=NTZP110Auth);
   CREATE TABLE RAROC_VEICULOS_CPF AS
   SELECT A.NR_CPF_CNPJ,CD_CON,a.NR_CPR,A.NR_CPR,DS_TPO_PSS,DT_MVM_DWM,
          VR_LBR,QT_DD_ATS_PRL,QT_PRL,
   ...
   FROM CONNECTION TO con1 
      ( SELECT DISTINCT NR_CPF_CNPJ,DS_TPO_PSS,NR_CPR,VR_LBR,QT_DD_ATS_PRL,
                        DT_MVM_DWM,PC_TX_JRO,QT_PRL,VR_SLD_DVD,CD_CON,
						MAX(DT_MVM_DWM) OVER (PARTITION BY NR_CPF_CNPJ,NR_CPR) AS DATA_MAX ,
        ...
        WHERE DS_MDL_BACEN='FINANCIAMENTOS' 
			  ...
			  AND DS_TPO_PSS='PF'
        ORDER BY NR_CPF_CNPJ,NR_CPR 
        ) AS A
		LEFT JOIN PLD.PD_COP_MARCO19_23_04 AS B ON (A.NR_CPF_CNPJ=B.NR_CPF_CNPJ AND A.NR_CPR=b.NR_CPR)
		WHERE B.PD NE . AND A.DT_MVM_DWM = A.DATA_MAX
        GROUP BY A.NR_CPF_CNPJ,A.NR_CPR,DS_TPO_PSS,VR_LBR,QT_DD_ATS_PRL,
                 DT_MVM_DWM,QT_PRL,Risco,PC_TX_JRO,B.PD,CAT_PRAZO,CD_CON,a.NR_CPR
        ORDER BY A.NR_CPF_CNPJ,A.NR_CPR ;
QUIT;
...
PROC STANDARD
   DATA=RAROC_VEICULOS_CPF(KEEP=NR_CPF_CNPJ VR_SLD_DVD CAT_PRAZO RISCO_BC)
