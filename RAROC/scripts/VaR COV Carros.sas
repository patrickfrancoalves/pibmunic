...
/*-------------------------------------------------------------------*/
/*---      CONSULTA DE UMA BOA COOPERATIVA EM JOÃO PESSOA         ---*/
/*-------------------------------------------------------------------*/
...
LIBNAME CONTAC  "/dados/RISCO/Modelagem_Nas/PCF_MODELAGEM/Dados_Conta_Corrente";


PROC SQL;
CONNECT TO NETEZZA AS CON1(DATABASE=PRD_DWM SERVER=NTZP107 AUTHDOMAIN=NTZP107AUTH);
CREATE TABLE INST AS 
SELECT *
FROM CONNECTION TO CON1
   ( SELECT DISTINCT SUBSTR(NUMCNPJCOOPERATIVA,1,8) AS CNPJ
       FROM INSTITUICAO AS A
       WHERE NM_CNT LIKE '%NORDESTE%' AND DS_TPO_CND='FILIADA'
       AND DT_MVM_DWM BETWEEN '2023-01-01' AND '2023-06-30' 
    )  WHERE CNPJ NOT IN ('00000000','-2')
       ORDER BY CNPJ;
...
SELECT DISTINCT NUMCNPJ_RAIZ,NUMCNPJFILIAL,UF,NOME_MUNICIPIO,NOME_FANTASIA
FROM RECEITA.TB_RFB_FILIAIS
WHERE NOME_MUNICIPIO IN ('JOAO PESSOA','CABEDELO') AND UF='PB' 
      AND NUMCNPJ_RAIZ IN (SELECT CNPJ FROM INST);
QUIT;


PROC SQL;
CONNECT TO NETEZZA AS CON1(DATABASE=PRD_DWM SERVER=NTZP107 AUTHDOMAIN=NTZP107AUTH);
CREATE TABLE JOAOPESSOA AS 
SELECT *
FROM CONNECTION TO CON1
   ( SELECT DISTINCT NR_CNT,NM_CNT,NR_CPR,SG_CNT,NM_CPR,DS_CPR,SG_CPR
       FROM INSTITUICAO AS A
       WHERE NM_CNT LIKE '%NORDESTE%' AND DS_TPO_CND='FILIADA'
       AND DT_MVM_DWM BETWEEN '2023-01-01' AND '2023-06-30' 
	   AND SUBSTR(NUMCNPJCOOPERATIVA,1,8) IN ('02275781','03102185','11907520','70116611')
    )  ORDER BY NR_CPR;
DISCONNECT FROM CON1;
QUIT;
