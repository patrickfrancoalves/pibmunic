...
Random layers are arrays of hidden unit activations that are random functions 
of input activation values (dot products for simple activation functions, 
distances from prototypes for radial basis functions).
"""
...
__all__ = ['RandomLayer', 'MLPRandomLayer', 'RBFRandomLayer', 'GRBFRandomLayer']
...
class BaseRandomLayer(BaseEstimator, TransformerMixin):
    ...
    # input_activations_
    def __init__(self, n_hidden=20, random_state=0, activation_func=None, activation_args=None):
...
class RandomLayer(BaseRandomLayer):
    ...
    try:
        ...
        _sigmoid = (lambda x: 1.0/(1.0 + logistic.cdf(-x)))
        


     
    #import bigfloat
    #_sigmoid = (lambda x: 1.0/(1.0 + bigfloat.exp(-x,bigfloat.precision(100)) ))
